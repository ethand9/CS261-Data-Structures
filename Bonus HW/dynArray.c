...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	assert(heap != 0);
  	...
  	int idxLeft = (pos*2) + 1;
  	int idxRight = (pos*2) + 2;
  	int newestNode;

  	if(idxRight < max){
  		newestNode = _smallerIndexHeap(heap, idxLeft, idxRight);
  		if(compare(getDynArr(heap, pos), getDynArr(heap, newestNode)) == 1){
  			swapDynArr(heap, pos, newestNode);
  			pos = newestNode;
  		...
  		if(compare(getDynArr(heap, pos), getDynArr(heap, idxLeft)) == 1){
  			swapDynArr(heap, pos, idxLeft);
  			pos = idxLeft;
  			_adjustHeap(heap, max, pos);
  		}
  		else
