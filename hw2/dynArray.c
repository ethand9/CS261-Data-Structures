...
void _dynArrSetCapacity(DynArr *v, int newCap)
{
	...
	assert(v != NULL);
	int i;
	TYPE *oldData = v->data; /* copy old data */
	int oldSize = v->size; /* copy old size */
...
void addDynArr(DynArr *v, TYPE val)
{
	/* FIXME: You will write this function */
	assert(v != NULL);
	/* double size if reached capacity */
	if(v->size >= v->capacity)
		_dynArrSetCapacity(v, v->capacity * 2);
	
	/* go to element after the last value and put new value */
	v->data[v->size] = val; 
	v->size++;
}
...
void removeAtDynArr(DynArr *v, int idx)
{
	...
	int i;
	/* start at removal position and loop until one before end*/
	/* move the data down one element starting from idx */
...
int containsDynArr(DynArr *v, TYPE val)
{
	/* FIXME: You will write this function */
	assert(v != NULL);
	/* if empty then does not contain */
	if(isEmptyDynArr(v))
		return 0;
	
	int i;
	/* loop through array and if find value then it contains */
	for(i = 0; i < v->size; i++)
...
void removeDynArr(DynArr *v, TYPE val)
{
	...
	int i;
	/* loop through array to find value then remove it */
	for(i = 0; i < v->size; i++)
		if(getDynArr(v,i) == val){
			removeAtDynArr(v,i);
			return;
